...
int main (int argc, char *argv[])
{
		...
		if (pid == 0)
		  {
			...
			if ( i == atoi(argv[1])-1){
				...
				if (pid == 0)
				{
					sprintf (buf,"%f", difftime(end_t,start_t));
					execlp("./elapsed","elapsed",buf, (char *) NULL);
					exit (-1);
				}
				waitpid (-1, NULL, 0);
			}
	...
	pid = fork();
	if (pid == 0) {
	execlp("sh","sh","ls_shell.sh",(char *) NULL);	//FUNCIONA :)
	exit(-1);
	}
	waitpid(-1,NULL,0);
	...
	pid = fork();
	if (pid == 0) {
		execlp ("make", "make","MOU_LOGS", (char *) NULL);
		exit(-1);
	}
	waitpid(-1,NULL,0);
